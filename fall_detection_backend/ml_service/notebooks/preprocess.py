...
# trim แยกต่อ class — เก็บ post-fall duration ที่แบ่ง A/B/C
FALL_A_TRIM           = 1200   # < 10 วิ → เก็บ 12 วิ (buffer)
FALL_B_TRIM           = 6500   # 10-60 วิ → เก็บ 65 วิ (buffer)
FALL_C_TRIM           = 9000   # > 60 วิ → เก็บ 90 วิ
NON_FALL_TRIM_SAMPLES = 1200   # เท่ากับ fall_A

# Multi-class label map: แยก fall_A/B/C เป็น class ของตัวเอง
LABEL_MAP = {
    "fall_A":   0,
    "fall_B":   1,
    "fall_C":   2,
...
CLASS_NAMES = {0: "fall_A", 1: "fall_B", 2: "fall_C", 3: "non_fall"}
N_CLASSES   = 4
...
def process_csv(csv_path: str):
    ...
    trim_map = {0: FALL_A_TRIM, 1: FALL_B_TRIM, 2: FALL_C_TRIM, 3: NON_FALL_TRIM_SAMPLES}
    trim = trim_map[label_int]
    if len(amplitudes) > trim:
        amplitudes = amplitudes[:trim]
...
def main():
    ...
    print(f"  ✂️   fall_A trim : {FALL_A_TRIM} pkts ({FALL_A_TRIM/100:.0f} วิ)  [< 10 วิ]")
    print(f"  ✂️   fall_B trim : {FALL_B_TRIM} pkts ({FALL_B_TRIM/100:.0f} วิ)  [10-60 วิ]")
    print(f"  ✂️   fall_C trim : {FALL_C_TRIM} pkts ({FALL_C_TRIM/100:.0f} วิ)  [> 60 วิ]")
    print(f"  ✂️   non_fall    : {NON_FALL_TRIM_SAMPLES} pkts ({NON_FALL_TRIM_SAMPLES/100:.0f} วิ)")
    print("=" * 62)
    print()
    print("  Label mapping:")
    ...
    meta = {
        ...
        "fall_a_trim": FALL_A_TRIM,
        "fall_b_trim": FALL_B_TRIM,
        "fall_c_trim": FALL_C_TRIM,
        ...
        "class_counts":         {CLASS_NAMES[k]: int(v) for k, v in class_counts.items()},
